...
const getOverlap = (tuple) => {
    const arrA = tuple[0].split('')
    const arrB = tuple[1].split('')
    const setA = new Set(arrA)
    return arrB.find(val => setA.has(val))
}

const getPriority = (char) => {
    const isCapital = !(char === char.toLowerCase())
    return isCapital ? char.charCodeAt(0) - 38 : char.charCodeAt(0) - 96
}
